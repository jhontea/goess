...
func TestGetWithRoundTripper_Success(t *testing.T) {
	...
	url := "https://twitter.com/hacktiv8id" //this url can be anything
...
func TestGetWithRoundTripper_Failure(t *testing.T) {
	...
	api := clientCall{*client}
	url := "https://invalid_url/hacktiv8" //we passed an invalid url
	body, err := api.GetValue(url)
	assert.NotNil(t, err)
	assert.Nil(t, body)
	assert.EqualValues(t, `Get "https://invalid_url/hacktiv8": we couldn't access the url provided`, err.Error())
}
